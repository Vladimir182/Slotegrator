...
class ServerLogs extends Component {
	constructor(props) {
		...
		const savedStartDate = sessionStorage.getItem('StartDateApi');
		const savedEndDate = sessionStorage.getItem('EndDateApi');
		const savedTypeLogs = sessionStorage.getItem('TypeLogsApi');
		const savedLevel = sessionStorage.getItem('LevelApi');
		const savedTextFilterValue = sessionStorage.getItem('LogsApiBodyTextFilterValue');
	...
	componentDidMount() {
		if( !sessionStorage.getItem('StartDateApi') ){
			sessionStorage.setItem('StartDateApi', moment(tm().utc().startOf('month').format('YYYY-MM-DD HH:mm:ss')))			
		}	
		if( !sessionStorage.getItem('EndDateApi') ){
			sessionStorage.setItem('EndDateApi', moment(tm().utc().endOf('day').format('YYYY-MM-DD HH:mm:ss')))
		}	
		...
		this.setState(
			{
				...
				sessionStorage.removeItem('StartDateApi');
				sessionStorage.removeItem('EndDateApi');
				sessionStorage.removeItem('TypeLogsApi');
				sessionStorage.removeItem('LevelApi');
				sessionStorage.removeItem('LogsApiBodyTextFilterValue');
	...
	handleDateStartChange = date => {
		...
		if (moment(date).isValid() && isDataRangeValid) {
			sessionStorage.setItem('StartDateApi', date);
	...
	handleDateEndChange = date => {
		...
		if (moment(date).isValid() && isDataRangeValid) {
			sessionStorage.setItem('EndDateApi', date);
	...
	handleChangeType = event => {
		sessionStorage.setItem('TypeLogsApi', event.target.value);
		this.setState(
			{
				...this.state,
	...
	handleChangeTypeLevel = event => {
		sessionStorage.setItem('LevelApi', event.target.value);
		this.setState(
			{
				...this.state,
	...
	handleTextFilterChange = e => {
		...
		this.setState({
				...
				sessionStorage.setItem('LogsApiBodyTextFilterValue', value);
	...
	handleUpdateData = (date) => {
		this.setState({
			startDate: sessionStorage.getItem('StartDateApi'),
			endDate: sessionStorage.getItem('EndDateApi')
		}, () => {
			this.handleDateStartChange(moment(sessionStorage.getItem('StartDateApi')))
			this.handleDateEndChange(moment(sessionStorage.getItem('EndDateApi')))
...
const mapStateToProps = state => ({
	logsTypes: state.logs.logsTypes.api,
	levels: state.logs.levels.api,
